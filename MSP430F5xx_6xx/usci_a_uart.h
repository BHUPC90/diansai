...
typedef struct USCI_A_UART_initParam {
    ...
    //! \n Valid values are:
    //! - \b USCI_A_UART_NO_PARITY [Default] 无奇偶校验
    //! - \b USCI_A_UART_ODD_PARITY 奇偶校验
    //! - \b USCI_A_UART_EVEN_PARITY 偶校验
    uint8_t parity;
    //! Controls direction of receive and transmit shift register.
    //! \n Valid values are:
    //! - \b USCI_A_UART_MSB_FIRST MSB优先
    //! - \b USCI_A_UART_LSB_FIRST [Default] LSB优先
    uint8_t msborLsbFirst;
    //! Indicates one/two STOP bits
    //! \n Valid values are:
    //! - \b USCI_A_UART_ONE_STOP_BIT [Default] 一个停止位
    //! - \b USCI_A_UART_TWO_STOP_BITS 两个停止位
    uint8_t numberofStopBits;
    //! Selects the mode of operation
    //! \n Valid values are:
    //! - \b USCI_A_UART_MODE [Default]
    //! - \b USCI_A_UART_IDLE_LINE_MULTI_PROCESSOR_MODE  空闲线路多处理器模式
    //! - \b USCI_A_UART_ADDRESS_BIT_MULTI_PROCESSOR_MODE  地址位多处理器模式
    //! - \b USCI_A_UART_AUTOMATIC_BAUDRATE_DETECTION_MODE  自动波特率检测模式
    uint8_t uartMode;
    //! Indicates low frequency or oversampling baud generation
    //! \n Valid values are:
    //! - \b USCI_A_UART_OVERSAMPLING_BAUDRATE_GENERATION  过采样波特率生成
    //! - \b USCI_A_UART_LOW_FREQUENCY_BAUDRATE_GENERATION  低频波特率生成
